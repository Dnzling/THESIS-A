...
/*!50001 DROP VIEW IF EXISTS `inventory_transactions_with_avg`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `inventory_transactions_with_avg` AS SELECT 
 1 AS `id`,
 ...
 1 AS `quantity_before`,
 1 AS `quantity_change`,
 1 AS `quantity_after`,
 1 AS `related_branch_id`,
 1 AS `reference_type`,
 1 AS `reference_id`,
 1 AS `notes`,
 1 AS `unit_cost`,
...
/*!50001 VIEW `inventory_transactions_with_avg` AS select `t`.`id` AS `id`,`t`.`transaction_number` AS `transaction_number`,`t`.`store_id` AS `store_id`,`t`.`branch_id` AS `branch_id`,`t`.`product_id` AS `product_id`,`t`.`variation_id` AS `variation_id`,`t`.`transaction_type` AS `transaction_type`,`t`.`quantity_before` AS `quantity_before`,`t`.`quantity_change` AS `quantity_change`,`t`.`quantity_after` AS `quantity_after`,`t`.`related_branch_id` AS `related_branch_id`,`t`.`reference_type` AS `reference_type`,`t`.`reference_id` AS `reference_id`,`t`.`notes` AS `notes`,`t`.`unit_cost` AS `unit_cost`,`t`.`total_value` AS `total_value`,`t`.`requires_approval` AS `requires_approval`,`t`.`approval_status` AS `approval_status`,`t`.`approval_workflow_id` AS `approval_workflow_id`,`t`.`approved_by` AS `approved_by`,`t`.`approved_at` AS `approved_at`,`t`.`created_by` AS `created_by`,`t`.`transaction_date` AS `transaction_date`,`t`.`created_at` AS `created_at`,`t`.`updated_at` AS `updated_at`,(select (sum(`t2`.`total_value`) / nullif(sum(`t2`.`quantity_change`),0)) from `inventory_transactions` `t2` where ((`t2`.`branch_id` = `t`.`branch_id`) and (`t2`.`product_id` = `t`.`product_id`) and ((`t2`.`variation_id` = `t`.`variation_id`) or ((`t2`.`variation_id` is null) and (`t`.`variation_id` is null))) and (`t2`.`transaction_date` <= `t`.`transaction_date`) and (`t2`.`approval_status` = 'approved'))) AS `running_avg_cost` from `inventory_transactions` `t` */;
...
-- ------------------------------------------------------
-- Thesis presentation seed data (generated)
-- Notes:
-- 1) Keeps global roles (store_id IS NULL) and preserves their IDs.
-- 2) Does not modify navigation_items, permissions, or role_permissions records.
-- 3) Demo login password for all seeded users: password123
-- ------------------------------------------------------

SET FOREIGN_KEY_CHECKS=0;

-- Preserve/create core global roles with fixed IDs and no store_id
INSERT INTO `roles` (`id`,`name`,`display_name`,`code`,`is_active`,`created_at`,`updated_at`,`description`,`store_id`) VALUES
(1,'super_admin','Super Admin','SUPER_ADMIN',1,'2026-01-10 09:00:00','2026-01-10 09:00:00','Global super admin',NULL),
(2,'store_admin','Store Admin','STORE_ADMIN',1,'2026-01-10 09:00:00','2026-01-10 09:00:00','Store administrator',NULL),
(3,'hr_manager','HR Manager','HR_MANAGER',1,'2026-01-10 09:00:00','2026-01-10 09:00:00','Human resources manager',NULL),
(4,'sales_staff','Sales Staff','SALES_STAFF',1,'2026-01-10 09:00:00','2026-01-10 09:00:00','Sales associate',NULL),
(5,'inventory_clerk','Inventory Clerk','INVENTORY_CLERK',1,'2026-01-10 09:00:00','2026-01-10 09:00:00','Inventory and warehouse staff',NULL)
...
-- Subscription plans for pricing/presentation
INSERT INTO `subscription_plans` (`id`,`plan_key`,`name`,`description`,`monthly_price`,`yearly_price`,`features`,`is_featured`,`is_active`,`sort_order`,`created_at`,`updated_at`) VALUES
(1,'simple','Simple Plan','Great for small furniture teams starting digital operations',1999.00,19190.00,'["Inventory","Sales POS","Basic Reports","Email Support"]',0,1,1,'2026-01-15 10:00:00','2026-01-15 10:00:00'),
(2,'unlimited','Unlimited Plan','Full platform access across all modules with priority support',4999.00,47990.00,'["Inventory","Sales POS","Procurement","Finance","HR Payroll","Ecommerce","Logistics","Supplier Portal","Advanced Analytics","Priority Support"]',1,1,2,'2026-01-15 10:00:00','2026-01-15 10:00:00')
ON DUPLICATE KEY UPDATE
`name`=VALUES(`name`),
`description`=VALUES(`description`),
...
-- Demo stores (anonymized but realistic)
INSERT INTO `stores` (`id`,`name`,`store_code`,`province`,`type`,`city`,`address`,`latitude`,`longitude`,`phone`,`email`,`website`,`status`,`subscription_tier`,`subscription_ends_at`,`settings`,`procurement_mode`,`procurement_threshold`,`central_procurement_managed_by`,`created_at`,`updated_at`) VALUES
(1,'Northwood Living','NWL-001','Metro Manila','retail','Quezon City','Timber Ave, Quezon City',14.67600000,121.04370000,'09170000001','hello@northwoodliving.demo','https://northwoodliving.demo','active','enterprise','2027-03-31','{"trial": false, "contact_person": "Ariana Cruz", "enabled_modules": ["inventory","procurement","sales","hr","logistics","finance","supplier","ecommerce"]}','hybrid',50000.00,'procurement_department','2025-11-15 09:00:00','2026-03-10 14:30:00'),
(2,'Harborline Furnishings','HBF-002','Cavite','retail','Bacoor','Coastline Rd, Bacoor',14.45000000,120.94000000,'09170000002','contact@harborlinefurnishings.demo','https://harborlinefurnishings.demo','active','premium','2026-12-31','{"trial": false, "contact_person": "Miguel Santos", "enabled_modules": ["inventory","procurement","sales","finance","hr","ecommerce"]}','hybrid',50000.00,'procurement_department','2025-12-01 10:00:00','2026-03-12 10:20:00'),
(3,'Cedar & Co. Home','CDC-003','Laguna','retail','Santa Rosa','Greenfield Blvd, Santa Rosa',14.31230000,121.11170000,'09170000003','support@cedarcohome.demo','https://cedarcohome.demo','active','basic','2026-10-31','{"trial": false, "contact_person": "Bianca Reyes", "enabled_modules": ["inventory","sales","finance","ecommerce"]}','distributed',45000.00,'store_admin','2025-12-20 08:45:00','2026-03-15 13:40:00'),
(4,'Summit Loft Interiors','SLI-004','Batangas','retail','Lipa','Summit Park, Lipa',13.94200000,121.16200000,'09170000004','team@summitloftinteriors.demo','https://summitloftinteriors.demo','active','free',NULL,'{"trial": true, "contact_person": "Noah Dela Fuente", "enabled_modules": ["inventory","procurement","sales","hr","logistics","finance","supplier","ecommerce"]}','hybrid',50000.00,'procurement_department','2026-01-08 11:15:00','2026-03-20 16:15:00')
ON DUPLICATE KEY UPDATE
`name`=VALUES(`name`),
...
-- Branches (main + satellite)
INSERT INTO `branches` (`id`,`store_id`,`name`,`address`,`city`,`barangay`,`province`,`latitude`,`longitude`,`geofence_radius_m`,`contact_number`,`branch_code`,`is_main_branch`,`branch_type`,`status`,`created_at`,`updated_at`) VALUES
(1,1,'Northwood Main Showroom','Timber Ave, QC','Quezon City','Bagumbayan','Metro Manila',14.67610000,121.04390000,30,'09170001001','NWL-MAIN',1,'storefront','active','2025-11-16 09:00:00','2026-03-10 14:30:00'),
(2,1,'Northwood Warehouse Hub','Industry Rd, QC','Quezon City','Bagong Silangan','Metro Manila',14.70010000,121.08020000,50,'09170001002','NWL-WH1',0,'warehouse','active','2025-11-20 09:00:00','2026-03-10 14:30:00'),
(3,2,'Harborline Main Branch','Coastline Rd, Bacoor','Bacoor','Mambog','Cavite',14.45020000,120.94040000,25,'09170002001','HBF-MAIN',1,'storefront','active','2025-12-02 10:00:00','2026-03-12 10:20:00'),
(4,2,'Harborline South Warehouse','Seaside Park, Bacoor','Bacoor','Talaba','Cavite',14.42990000,120.93200000,40,'09170002002','HBF-WH1',0,'warehouse','active','2025-12-05 10:00:00','2026-03-12 10:20:00'),
(5,3,'Cedar Main Branch','Greenfield Blvd, Santa Rosa','Santa Rosa','Balibago','Laguna',14.31250000,121.11190000,25,'09170003001','CDC-MAIN',1,'storefront','active','2025-12-21 08:45:00','2026-03-15 13:40:00'),
(6,4,'Summit Main Branch','Summit Park, Lipa','Lipa','Banay-Banay','Batangas',13.94210000,121.16210000,25,'09170004001','SLI-MAIN',1,'storefront','active','2026-01-09 11:15:00','2026-03-20 16:15:00')
ON DUPLICATE KEY UPDATE
`name`=VALUES(`name`),
`address`=VALUES(`address`),
`city`=VALUES(`city`),
...
-- Users: all passwords set to password123 (bcrypt)
INSERT INTO `users` (`id`,`user_id`,`fname`,`lname`,`email`,`pending_email`,`birthday`,`store_id`,`branch_id`,`email_verified_at`,`password`,`role_id`,`is_active`,`otp_code`,`otp_expires_at`,`registered_by`,`remember_token`,`last_login_at`,`created_at`,`updated_at`,`deleted_at`) VALUES
(1,'USR-000001','System','Administrator','admin@furnisync.demo',NULL,'1990-01-01',NULL,NULL,'2026-01-10 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',1,1,NULL,NULL,NULL,NULL,'2026-04-01 08:00:00','2026-01-10 09:00:00','2026-04-01 08:00:00',NULL),
(2,'USR-000002','Ariana','Cruz','ariana.cruz@northwoodliving.demo',NULL,'1992-03-14',1,1,'2026-01-12 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',2,1,NULL,NULL,1,NULL,'2026-04-03 09:15:00','2026-01-12 09:00:00','2026-04-03 09:15:00',NULL),
(3,'USR-000003','Miguel','Santos','miguel.santos@harborlinefurnishings.demo',NULL,'1989-11-07',2,3,'2026-01-13 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',2,1,NULL,NULL,1,NULL,'2026-04-02 10:10:00','2026-01-13 09:00:00','2026-04-02 10:10:00',NULL),
(4,'USR-000004','Bianca','Reyes','bianca.reyes@cedarcohome.demo',NULL,'1993-05-22',3,5,'2026-01-14 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',2,1,NULL,NULL,1,NULL,'2026-04-01 11:45:00','2026-01-14 09:00:00','2026-04-01 11:45:00',NULL),
(5,'USR-000005','Noah','Dela Fuente','noah.delafuente@summitloftinteriors.demo',NULL,'1991-08-16',4,6,'2026-01-15 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',2,1,NULL,NULL,1,NULL,'2026-04-04 13:20:00','2026-01-15 09:00:00','2026-04-04 13:20:00',NULL),
(6,'USR-000006','Leah','Navarro','leah.navarro@northwoodliving.demo',NULL,'1994-02-02',1,1,'2026-01-18 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',3,1,NULL,NULL,2,NULL,'2026-04-03 08:40:00','2026-01-18 09:00:00','2026-04-03 08:40:00',NULL),
(7,'USR-000007','Jared','Lim','jared.lim@northwoodliving.demo',NULL,'1996-09-30',1,1,'2026-01-18 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',4,1,NULL,NULL,2,NULL,'2026-04-03 08:43:00','2026-01-18 09:05:00','2026-04-03 08:43:00',NULL),
(8,'USR-000008','Paolo','Garcia','paolo.garcia@northwoodliving.demo',NULL,'1995-07-11',1,2,'2026-01-18 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',5,1,NULL,NULL,2,NULL,'2026-04-03 08:49:00','2026-01-18 09:10:00','2026-04-03 08:49:00',NULL),
(9,'USR-000009','Ivy','Torres','ivy.torres@harborlinefurnishings.demo',NULL,'1994-12-01',2,3,'2026-01-20 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',3,1,NULL,NULL,3,NULL,'2026-04-02 10:20:00','2026-01-20 09:00:00','2026-04-02 10:20:00',NULL),
(10,'USR-000010','Ralph','Santiago','ralph.santiago@harborlinefurnishings.demo',NULL,'1997-10-12',2,3,'2026-01-20 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',4,1,NULL,NULL,3,NULL,'2026-04-02 10:21:00','2026-01-20 09:05:00','2026-04-02 10:21:00',NULL),
(11,'USR-000011','Karen','Uy','karen.uy@harborlinefurnishings.demo',NULL,'1995-04-18',2,4,'2026-01-20 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',5,1,NULL,NULL,3,NULL,'2026-04-02 10:25:00','2026-01-20 09:10:00','2026-04-02 10:25:00',NULL),
(12,'USR-000012','Nico','Aquino','nico.aquino@cedarcohome.demo',NULL,'1993-06-09',3,5,'2026-01-22 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',3,1,NULL,NULL,4,NULL,'2026-04-01 12:05:00','2026-01-22 09:00:00','2026-04-01 12:05:00',NULL),
(13,'USR-000013','Mara','Valdez','mara.valdez@cedarcohome.demo',NULL,'1998-02-27',3,5,'2026-01-22 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',4,1,NULL,NULL,4,NULL,'2026-04-01 12:10:00','2026-01-22 09:05:00','2026-04-01 12:10:00',NULL),
(14,'USR-000014','Dane','Rivera','dane.rivera@cedarcohome.demo',NULL,'1996-01-19',3,5,'2026-01-22 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',5,1,NULL,NULL,4,NULL,'2026-04-01 12:15:00','2026-01-22 09:10:00','2026-04-01 12:15:00',NULL),
(15,'USR-000015','Trisha','Mendoza','trisha.mendoza@summitloftinteriors.demo',NULL,'1994-09-05',4,6,'2026-01-25 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',3,1,NULL,NULL,5,NULL,'2026-04-04 13:25:00','2026-01-25 09:00:00','2026-04-04 13:25:00',NULL),
(16,'USR-000016','Jon','Flores','jon.flores@summitloftinteriors.demo',NULL,'1999-10-10',4,6,'2026-01-25 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',4,1,NULL,NULL,5,NULL,'2026-04-04 13:30:00','2026-01-25 09:05:00','2026-04-04 13:30:00',NULL),
(17,'USR-000017','Gia','Lopez','gia.lopez@summitloftinteriors.demo',NULL,'1997-03-03',4,6,'2026-01-25 09:00:00','$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',5,1,NULL,NULL,5,NULL,'2026-04-04 13:31:00','2026-01-25 09:10:00','2026-04-04 13:31:00',NULL)
ON DUPLICATE KEY UPDATE
`fname`=VALUES(`fname`),
`lname`=VALUES(`lname`),
`email`=VALUES(`email`),
...
-- Unlimited/simple onboarding profiles (6-month realistic timeline)
INSERT INTO `trial_onboarding_profiles` (`id`,`user_id`,`plan`,`employee_range`,`branch_range`,`modules`,`primary_goal`,`first_team`,`completed_at`,`created_at`,`updated_at`) VALUES
(1,2,'unlimited','21-50','2-5','["inventory","procurement","sales","hr","logistics","finance","supplier","ecommerce"]','Scale omnichannel sales','Store Operations','2026-01-12 10:00:00','2026-01-12 09:40:00','2026-03-10 14:30:00'),
(2,3,'unlimited','21-50','2-5','["inventory","procurement","sales","hr","logistics","finance","supplier","ecommerce"]','Improve procurement controls','Finance and Procurement','2026-01-13 10:00:00','2026-01-13 09:40:00','2026-03-12 10:20:00'),
(3,4,'simple','6-20','1-2','["inventory","sales","finance","ecommerce"]','Strengthen POS performance','Sales Team','2026-01-14 10:00:00','2026-01-14 09:40:00','2026-03-15 13:40:00'),
(4,5,'unlimited','6-20','1-2','["inventory","procurement","sales","hr","logistics","finance","supplier","ecommerce"]','Launch full-store digital operations','All Departments','2026-01-15 10:00:00','2026-01-15 09:40:00','2026-03-20 16:15:00')
ON DUPLICATE KEY UPDATE
`plan`=VALUES(`plan`),
`employee_range`=VALUES(`employee_range`),
`branch_range`=VALUES(`branch_range`),
`modules`=VALUES(`modules`),
`primary_goal`=VALUES(`primary_goal`),
`first_team`=VALUES(`first_team`),
`completed_at`=VALUES(`completed_at`),
`updated_at`=VALUES(`updated_at`);

-- Optional: update existing users with store_id to demo password for easy presentation access
UPDATE `users`
SET `password`='$2y$10$VDerpJbFttlMey19UAWmgOeTOPCiDxmbZlcGR1qopXRFCBBKtCMJG',

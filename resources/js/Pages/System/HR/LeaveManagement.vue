...
<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <!-- Day Leaves Dialog -->
    <Dialog v-model:visible="showDayDialog" modal :style="{ width: '520px' }" class="rounded-xl">
      <template #header>
        <div class="flex items-center gap-2 text-gray-700">
          <i class="pi pi-calendar text-blue-500"></i>
          <span class="font-semibold">Leaves on {{ selectedDayLabel }}</span>
        </div>
      </template>

      <div v-if="dayLeaves.length === 0" class="py-6 text-center text-gray-400">
        <i class="pi pi-calendar-times text-4xl mb-2 block"></i>
        <p>No approved leaves on this date.</p>
      </div>

      <div v-else class="space-y-3">
        <div v-for="leave in dayLeaves" :key="leave.id" class="border border-gray-100 rounded-lg p-3">
          ...
          <div class="text-sm text-gray-600 mt-2">
            {{ formatDateRange(leave.startDate, leave.endDate) }} • {{ leave.duration }} day(s)
          </div>
          <div class="flex justify-end mt-3">
            <Button label="View" icon="pi pi-eye" size="small" severity="info" outlined
              @click="goToLeaveDetail(leave.id)" />
          </div>
        </div>
      </div>
    </Dialog>
...
const dayLeaves = ref<LeaveRequest[]>([])
...
const viewDayDetails = (day: any) => {
  if (!day?.date || !day.hasLeave) return
  const target = new Date(day.date)
  target.setHours(0, 0, 0, 0)

  dayLeaves.value = leaveRequests.value.filter(r => {
    if (r.status !== 'approved') return false
    const startDate = new Date(r.start_date)
    startDate.setHours(0, 0, 0, 0)
...
const goToLeaveDetail = (id: number) => {

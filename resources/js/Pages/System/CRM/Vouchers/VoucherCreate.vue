    ...
    <Card class="border border-gray-100 shadow-sm rounded-2xl">
      <template #content>
        <div class="flex items-center justify-between">
          ...
          <Button text icon="pi pi-arrow-left" label="Back" @click="router.visit('/crm/vouchers')" />
        </div>
      </template>
    </Card>
    ...
    <Card class="border border-gray-100 shadow-sm rounded-2xl">
      <template #content>
        ...
        <div class="mt-6 flex justify-end gap-2">
          <Button text label="Cancel" @click="router.visit('/crm/vouchers')" />
...
const submit = async () => {
  ...
  try {
    const res = await crmService.createVoucher({
      ...form,
    ...
    if (id) {
      router.visit(`/crm/vouchers/${id}`)
      return
    }
    router.visit('/crm/vouchers')
  } catch (error: any) {

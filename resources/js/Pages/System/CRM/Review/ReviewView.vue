        ...
        <div v-else class="space-y-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <div class="text-lg font-semibold text-gray-900">{{ review.customer_name || 'â€”' }}</div>
          ...
          <div>
            ...
            <div class="text-sm text-gray-700">{{ review.message || 'â€”' }}</div>
          </div>
...
const loadReview = async () => {
  loading.value = true
  try {
    const res = await crmService.getReview(String(route.params.id))
...
const saveReply = async () => {
  try {
    await crmService.replyReview(String(route.params.id), { reply: replyText.value })

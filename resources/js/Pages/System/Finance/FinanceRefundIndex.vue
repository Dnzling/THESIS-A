...
const router=useRouter(),toast=useToast(),auth=useAuthStore(); const canManage=auth.hasPermission('finance.refunds.manage'); const loading=ref(false),refunds=ref<any[]>([]); const filters=reactive({search:'',source:'all',status:''}); const sources=[{label:'All Sources',value:'all'},{label:'Ecommerce Returns',value:'ecommerce_return'},{label:'Ecommerce',value:'ecommerce'},{label:'In Store',value:'sales'},{label:'Manual',value:'manual'}]; const statuses=[{label:'All Statuses',value:''},{label:'Pending Inspection',value:'pending_inspection'},{label:'Pending Finance Approval',value:'pending'},{label:'Approved',value:'approved'},{label:'Rejected',value:'rejected'}]
const currency=(v:any)=>new Intl.NumberFormat('en-PH',{style:'currency',currency:'PHP'}).format(Number(v||0)); const date=(v:any)=>v?new Date(v).toLocaleDateString('en-PH',{month:'short',day:'numeric',year:'numeric'}):'-'; const label=(v:any)=>String(v||'pending').replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase()); const sourceLabel=(v:string)=>({ecommerce_return:'Ecommerce Return',ecommerce:'Ecommerce',sales:'In Store',manual:'Manual'} as any)[v]||label(v); const severity=(v:string)=>v==='approved'?'success':v==='rejected'?'danger':v==='pending_inspection'?'info':'warn'; const summary=computed(()=>[{label:'Pending Inspection',value:refunds.value.filter(x=>x.status==='pending_inspection').length,icon:'pi pi-search',color:'text-blue-600'},{label:'Pending Approval',value:refunds.value.filter(x=>x.status==='pending').length,icon:'pi pi-clock',color:'text-amber-600'},{label:'Approved',value:refunds.value.filter(x=>x.status==='approved').length,icon:'pi pi-check-circle',color:'text-emerald-600'},{label:'Pending Amount',value:refunds.value.filter(x=>['pending','pending_inspection'].includes(x.status)).reduce((s,x)=>s+Number(x.amount||0),0),money:true,icon:'pi pi-wallet',color:'text-rose-600'}]);

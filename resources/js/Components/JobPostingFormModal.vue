  ...
  >
    ...
    <form class="space-y-6" @submit.prevent="handleSubmit">
          ...
          <Card class="border border-surface-200 shadow-none">
            <template #content>
              <div class="grid gap-5 md:grid-cols-2">
                ...
                <div class="space-y-2 md:col-span-2">
                  <label class="text-sm font-medium text-surface-700">Salary Range</label>
                  <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
                    <InputNumber
                      ...
                      placeholder="Minimum"
                    />
                    <div class="flex items-center justify-center text-xs font-semibold text-surface-500">to</div>
                    <InputNumber
                  ...
                  <small v-else-if="showFieldError('salary_max')" class="text-red-500">Maximum salary is required.</small>
                  <small v-else-if="salaryRangeInvalid" class="text-amber-600">Maximum salary should be higher than minimum.</small>
                </div>

                <div class="space-y-2 md:col-span-2">
                  <label class="text-sm font-medium text-surface-700">Description</label>
                  <div class="flex flex-wrap gap-2">
                    <Button type="button" size="small" outlined label="Sales Template" @click="applyTemplate('sales')" />
                    <Button type="button" size="small" outlined label="Design Template" @click="applyTemplate('design')" />
                    <Button type="button" size="small" outlined label="Operations Template" @click="applyTemplate('ops')" />
                  </div>
                  <Textarea
      ...
      <div class="flex w-full justify-end gap-3 pt-2">
        ...
        <Button type="submit" severity="info" :label="posting ? 'Update Job Posting' : 'Create Job Posting'" />
      </div>
    </form>
  </Dialog>
</template>
...
const filteredRoleOptions = computed(() => {
  ...
  return roleOptions.value.filter((role) => {
    ...
    return String(role.department).toLowerCase() === String(form.value.department).toLowerCase()
  })
})
...
const loadRoles = async (department?: string) => {
  loadingRoles.value = true
  try {
    const endpoint = department
      ? '/api/store/roles/scoped-by-department'
    ...
    roleOptions.value = items
      ...
      .map((role: any) => ({
        ...
        value: role.id,
        department: role.department || role.department_name || role.departmentTitle || '',
      }))
  } finally {
...
const applyTemplate = (type: 'sales' | 'design' | 'ops') => {
  const templates: Record<string, string> = {
    sales: `Key Responsibilities:
- Engage customers and recommend products
- Manage inquiries and follow-ups
...
Qualifications:
- Portfolio of interior or product design work
- Proficiency in design tools`,
    ops: `Key Responsibilities:
- Coordinate daily operations and inventory
- Support warehouse and fulfillment workflows
- Ensure process compliance and reporting

Qualifications:
- Strong organizational skills
- Experience with inventory systems`,
  }
  form.value.description = templates[type]
}
...
const handleRoleChange = () => {
  ...
  if (!newRoleId) return
  const role = roleOptions.value.find((r) => r.value === newRoleId)
  if (role?.label) {
    const currentTitle = form.value.title?.trim()
    if (!currentTitle || currentTitle === lastAutoTitle.value) {
      form.value.title = role.label
      lastAutoTitle.value = role.label
    }
  }
  if (role?.department && (!form.value.department || form.value.department !== role.department)) {
...
const handleDepartmentChange = () => {
  ...
  if (form.value.role_id) {
    const role = roleOptions.value.find((r) => r.value === form.value.role_id)
    if (role?.department && role.department !== form.value.department) {
      form.value.role_id = null
    }
  }
  loadRoles(form.value.department)
}

watch(() => form.value.role_id, handleRoleChange)

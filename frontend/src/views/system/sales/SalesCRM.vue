    ...
    <Card class="border border-gray-100 shadow-sm rounded-2xl">
      <template #content>
        <div class="flex items-center justify-between">
          ...
          <Button severity="info" icon="pi pi-plus" label="New Lead" @click="openCreate" />
        </div>
      </template>
    </Card>
          ...
          <Column header="Actions">
            <template #body="{data}">
              <div class="flex items-center gap-2">
                <Select v-model="data.__stage" :options="stageOptions" optionLabel="label" optionValue="value" class="w-40" />
                <Button text severity="info" label="Update" @click="updateStage(data)" />
              </div>
            </template>
          </Column>
        </DataTable>

  ...
  <div class="bg-gray-50 min-h-screen p-6">
    <div class="max-w-4xl mx-auto">
      <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Update unit information</p>
      </div>
...
const populateForm = () => {
  ...
  form.abbreviation = unit.value.abbreviation || ''
  form.type = unit.value.type || ''
  form.conversion_factor = unit.value.conversion_factor || null
  form.description = unit.value.description || ''
  form.status = unit.value.status || 'active'
  form.display_order = unit.value.display_order || 0
}
...
const submitForm = async () => {
  ...
  try {
    const response = await inventoryService.updateUnit(unit.value.id, form)
    ...
    if (response.success) {
      ...
      router.push({ name: 'inventory.units.detail', params: { id: unit.value.id } })
    } else {
    ...
    if (error.response?.status === 422) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Error',
        detail: error.response?.data?.message || 'Failed to update unit',
        life: 3000
      })
    }
  } finally {

      ...
      <div class="mb-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            />
            <div>
              <h1 class="text-3xl font-bold text-gray-800">Add Batch</h1>
              <p class="text-gray-600 mt-1">Create a new product batch for tracking and expiry management</p>
            </div>
          </div>
        </div>
      </div>
...
const submitForm = async () => {
  ...
  try {
    const response = await inventoryService.createBatch(form)
    ...
    if (response.success) {
      ...
      if (response.errors) {
        ...
        toast.add({
          severity: 'error',
          summary: 'Error',
          detail: response.message || 'Failed to create batch',
          life: 3000
        })
      }
    }
    ...
    if (error.response?.data?.errors) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Error',
        detail: error.response?.data?.message || 'Failed to create batch',
        life: 3000
      })
    }
  } finally {

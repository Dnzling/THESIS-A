<template>
  <div class="bg-gray-50 min-h-screen p-6">
    <div class="max-w-4xl mx-auto">
      <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Issue Stock</h1>
        <p class="text-gray-600 mt-1">Record a stock issue transaction</p>
      </div>
...
const maxQuantity = computed(() => {
  ...
  return stockLevels.value[key] || 0
})
...
const submitForm = async () => {
  ...
  try {
    const response = await inventoryService.createStockIssue(form)

    if (response.success) {
      toast.add({
        severity: 'success',
        summary: 'Success',
        detail: 'Stock issue recorded successfully',
        life: 3000
      })
    ...
    if (error.response?.status === 422) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Error',
        detail: error.response?.data?.message || 'Failed to record stock issue',
        life: 3000
      })
    }
  } finally {

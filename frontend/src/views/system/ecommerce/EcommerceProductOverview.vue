      ...
      <template #content>
        <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
          <div class="overflow-hidden rounded-2xl bg-slate-100">
            ...
            <div
              v-if="show3DViewer && selectedModel3D"
              class="relative min-h-90 w-full"
            >
              <Model3DPreview
            ...
            <div v-if="!show3DViewer && galleryImages.length > 1" class="border-t border-slate-200 bg-white p-3">
              <Carousel :value="galleryImages" :numVisible="4" :numScroll="1" :circular="false">
                <template #item="{ data }">
                  <button
                    type="button"
                    class="h-18 w-18 overflow-hidden rounded-lg border transition"
          ...
          <div class="space-y-4">
            ...
            <h1 class="text-3xl font-semibold text-slate-900">{{ product.product_name }}</h1>
            ...
            <div class="rounded-xl border border-slate-200 p-3">
              ...
              <div class="mt-2 grid grid-cols-2 gap-2 text-sm text-slate-700">
                <p>Length: <span class="font-semibold">{{ displayDimension(product?.dimensions?.length_cm, 'cm') }}</span></p>
                <p>Width: <span class="font-semibold">{{ displayDimension(product?.dimensions?.width_cm, 'cm') }}</span></p>
                <p>Height: <span class="font-semibold">{{ displayDimension(product?.dimensions?.height_cm, 'cm') }}</span></p>
                <p>Weight: <span class="font-semibold">{{ displayDimension(product?.dimensions?.weight_kg, 'kg') }}</span></p>
              </div>
            </div>
            ...
            <div v-if="product?.variations?.length" class="space-y-2">
              ...
              <p v-if="selectedVariation" class="text-sm text-slate-600">
                Variation Price: <span class="font-semibold">{{ formatCurrency(selectedVariation.final_price || product.price) }}</span>
                <span v-if="selectedVariation.model_3d" class="ml-2 text-xs text-emerald-600">(Has 3D model)</span>
              </p>
            </div>

            <div class="flex items-center gap-3 pt-2">
              <InputNumber v-model="quantity" :min="1" :max="Math.max(1, Number(product.quantity_available || 1))" showButtons />
              <Button label="Add to Cart" severity="info" :disabled="Number(product.quantity_available || 0) <= 0" @click="addToCart" />
            </div>
          </div>
        </div>
      </template>
    ...
    <div v-if="product" class="mt-6 rounded-2xl border border-slate-200 bg-white">
      <div class="flex flex-wrap gap-2 border-b border-slate-200 p-3">
        <Button
          ...
          size="small"
          :severity="activeTab === 'description' ? 'info' : 'secondary'"
          :outlined="activeTab !== 'description'"
          @click="activeTab = 'description'"
        />
        <Button
          label="Reviews"
          size="small"
          :severity="activeTab === 'reviews' ? 'info' : 'secondary'"
          :outlined="activeTab !== 'reviews'"
          @click="activeTab = 'reviews'"
        />
        <Button
          label="Recommended"
          size="small"
          :severity="activeTab === 'recommended' ? 'info' : 'secondary'"
          :outlined="activeTab !== 'recommended'"
      ...
      <div v-else-if="activeTab === 'reviews'" class="p-5">
        <div class="grid grid-cols-1 gap-6 border-b border-slate-200 pb-4 lg:grid-cols-12">
          <div class="lg:col-span-3">
            ...
            <div class="mt-3 inline-flex h-40 w-40 items-center justify-center rounded-full border-[12px] border-amber-400 text-center">
              <div>
...
const reviews = computed(() => {
  const base = Number(product.value?.id || 1)
  return [
    {
      id: 1,
      name: 'Liam Carter',
      ...
      rating: 5,
      comment: 'Absolutely fantastic quality. The material feels premium and very durable.',
    },
    {
      id: 2,
      name: 'Emma Wilson',
      ...
      rating: 4,
      comment: 'Great product and smooth checkout experience. Exactly as shown in photos.',
    },
    {
      id: 3,
      name: 'Noah Reyes',
      ...
      rating: (base % 2) + 4,
      comment: 'Comfortable and worth the price. Recommended for home use.',
    },
  ]
})

          ...
          <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="pb-6 border-b">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-700">Quantity Requested <span class="text-red-500">*</span></label>
                  <InputNumber
        ...
        <template #content>
          <div class="space-y-4">
            ...
            <Button
              ...
              @click="submitForm"
              :loading="submitting"
              :disabled="submitting || !form.product_id || !form.quantity_requested"
            />
          </div>
        </template>
      </Card>
...
const loadBranches = async () => {
  try {
    const response = await inventoryService.getBranches?.() || { data: [] }
  ...
  if (form.product_id) {
    ...
    if (product) {
      ...
      if (product.latestInventory) {
        currentStock.value = product.latestInventory.quantity_on_hand?.toString() || 'N/A'
        reorderPoint.value = product.latestInventory.reorder_point?.toString() || 'N/A'
      }
    }
  }
}

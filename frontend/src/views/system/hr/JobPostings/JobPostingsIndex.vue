<template>
  <div class="space-y-6">
    ...
    <section class="rounded-3xl border border-blue-100 bg-linear-to-r from-blue-700 via-blue-600 to-sky-500 px-6 py-7 text-white shadow-sm">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div class="space-y-2">
          <p class="text-xs font-semibold uppercase tracking-[0.24em] text-blue-100">HR Recruitment</p>
          <h1 class="text-3xl font-semibold tracking-tight">Job Postings</h1>
          <p class="max-w-2xl text-sm text-blue-50/90">
            Manage open roles, track applications, and maintain a cleaner hiring pipeline in one place.
          </p>
        </div>

        <Button
          v-if="hasPermission('create-job-postings')"
        ...
        <template #content>
          <div class="space-y-3">
            <div class="flex items-start justify-between gap-4">
              ...
              <div
                v-if="hasPermission('edit-job-postings')"
              ...
              >
                <Button
                  ...
                  text
                  rounded
                  @click.stop="editPosting(posting)"
                />
                <Button
                  icon="pi pi-trash"
                  severity="danger"
                  text
                  rounded
                  @click.stop="deletePosting(posting.id)"
                />
              </div>
            </div>

<template>
  <div class="min-h-full">
              ...
              <div class="space-y-4">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                  <div class="rounded-2xl bg-blue-50/80 p-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-blue-700">Current Position</p>
                    <p class="mt-2 text-sm font-semibold text-slate-900">{{ application?.current_position || 'Not provided' }}</p>
                  </div>
                  <div class="rounded-2xl bg-blue-50/80 p-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-blue-700">Current Company</p>
          ...
          <Card class="border border-slate-200 shadow-none">
            ...
            <template #content>
              <div v-if="application?.documents?.length" class="space-y-3">
                <div v-for="doc in application.documents" :key="doc.id" class="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div>
                  ...
                  <div class="flex gap-2">
                    ...
                    <Button label="Download" icon="pi pi-download" severity="secondary" outlined @click="downloadDocument(doc)" />
                  </div>
                </div>
              </div>
          ...
          <Card class="border border-blue-100 bg-blue-50/70 shadow-none">
            ...
            <template #content>
              ...
              <div v-else-if="canScheduleInterview" class="space-y-3">
                ...
                <Select v-model="interviewForm.interview_type" :options="interviewTypeOptions" optionLabel="label" optionValue="value" placeholder="Interview type" class="w-full" />
                <InputNumber v-model="interviewForm.duration_minutes" :min="15" :max="480" class="w-full" placeholder="Duration (minutes)" />
                <Textarea v-model="interviewForm.notes" rows="3" class="w-full" placeholder="Interview notes" />
                <Message v-if="errorMessage" severity="error" :closable="false">{{ errorMessage }}</Message>
                <Button label="Schedule Interview" icon="pi pi-calendar-plus" severity="info" fluid :loading="submitting" @click="scheduleInterview" />
              </div>

              <div v-else class="space-y-3">
                <p class="text-sm leading-6 text-slate-600">
                  This applicant is already in the decision-ready stage. Continue to the dedicated decision page for Hire or Reject.
                </p>
                <Button label="Open Decision Page" icon="pi pi-arrow-right" severity="info" fluid @click="router.push({ name: 'hr.job-applications.decision', params: { applicationId: route.params.applicationId } })" />
              </div>
            </template>
          </Card>
    ...
    <Dialog v-model:visible="previewVisible" modal maximizable :style="{ width: 'min(78rem, 96vw)' }" header="Document Preview">
...
const interviewTypeOptions = [
  ...
  { label: 'Practical Test', value: 'Practical Test' },
]
